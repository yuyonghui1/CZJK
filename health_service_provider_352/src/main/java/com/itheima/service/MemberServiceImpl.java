...
//    }
    ...
    public Map<String, Object> getMemberReport(String startDate, String endDate) {
        try {
            ...
            for (int i = 0; i < result + month; i++) {
                bef.add(Calendar.MONTH, 1);
                bef.set(Calendar.DAY_OF_MONTH, 1);
                bef.add(Calendar.MONTH, 1);
                bef.add(Calendar.DATE, -1);
                String Eachdate = simpleDateFormat.format(bef.getTime());
                months.add(Eachdate);
            }
            for (String date : months) {
                long count1 = memberDao.findCountByDate(date);
                memberCount.add(count1);
            }
            map.put("months", months);
            map.put("memberCount", memberCount);
            return map;
